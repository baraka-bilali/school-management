              ...
              {paymentsLoading ? (
                ...
                <>
                  <ul className={`divide-y ${borderColor}`}>
                    <li className={`grid grid-cols-[minmax(0,1fr)_7.5rem_auto] sm:grid-cols-[1fr_10rem_auto] gap-3 px-4 sm:px-6 py-2.5 text-[11px] font-semibold uppercase tracking-wide ${textSecondary}`}>
                      <span>Facture</span>
                      <span>Date</span>
                      <span className="text-right w-[4.75rem]">Action</span>
                    </li>
                    {payments.map((p) => {
                      const paymentDate = parseSubscriptionDate(p.createdAt) ?? new Date(p.createdAt)
                      const dateLabel = paymentDate.toLocaleDateString("fr-FR", {
                        day: "2-digit",
                        month: "short",
                        year: "numeric",
                      })
                      ...
                      <li
                        key={p.id}
                        className={`grid grid-cols-[minmax(0,1fr)_7.5rem_auto] sm:grid-cols-[1fr_10rem_auto] items-center gap-3 px-4 sm:px-6 py-3.5 transition-colors ${tableRowHover}`}
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-teal-500/10 text-teal-500">
                            <Receipt className="w-4 h-4" />
                          </div>
                          <span className="font-mono text-sm font-semibold text-teal-500 truncate">
                            {p.numeroFacture}
                          </span>
                        </div>
                        <span className={`inline-flex items-center gap-1.5 text-sm tabular-nums ${textColor}`}>
