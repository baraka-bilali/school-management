...
export default function StaffCommuniqueViewPage() {
  ...
  useEffect(() => {
    const fetchCommunique = async () => {
      try {
        const res = await fetch(`/api/staff/communiques/${params.id}`, { credentials: "include" })
        if (res.ok) {
  ...
  if (notFound || !communique) {
    return (
      <div className="flex min-h-[50vh] flex-col items-center justify-center text-center">
        ...
        <button type="button" onClick={() => router.push("/staff/messages")} className="mt-4 text-sm font-semibold text-indigo-600">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={cn("overflow-hidden rounded-2xl border", card, border, shadow)}>
        ...
        <div className="p-5">
          <div
            className={cn("prose prose-sm max-w-none dark:prose-invert", text)}
