 ...
 * Années scolaires auxquelles l'élève a été inscrit, avec marque « en cours ».
 */
export async function GET(req: NextRequest) {
  ...
  const [enrollments, currentYearId] = await Promise.all([
    prisma.enrollment.findMany({
      ...
      include: {
        year: { select: { id: true, name: true, current: true, startDate: true, endDate: true } },
  ...
  // Une inscription par année (unique studentId+yearId) — dédupliquer par sécurité
  ...
  // S'assurer que l'année en cours apparaît en premier si présente
  years.sort((a, b) => {
