...
export default function TeacherWalletPage() {
  ...
  const fetchPayments = useCallback(async (silent = false) => {
    ...
    try {
      const res = await fetch("/api/teacher/payments", { credentials: "include" })
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("teacherPaymentReceived", onPayment)
    return () => window.removeEventListener("teacherPaymentReceived", onPayment)
  }, [fetchPayments])
  ...
  const openInvoice = async (id: number) => {
    ...
    setInvoiceOpen(true)
    try {
      const res = await fetch(`/api/teacher/payments/${id}`, { credentials: "include" })
      if (res.ok) {
        const data = await res.json()
  ...
  return (
    <div className="space-y-5 lg:space-y-8">
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <p className={cn("font-semibold truncate", text)}>{p.typeLabel}</p>
      ...
      {/* Modal facture — même pattern que la déconnexion admin */}
      {invoiceOpen && (
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-3 text-sm sm:grid-cols-2">
                  <div>
                    ...
                    <p className={cn("font-medium", text)}>{invoice.teacherName}</p>
                    {invoice.specialty && <p className={textMuted}>{invoice.specialty}</p>}
                  </div>
                  <div>
