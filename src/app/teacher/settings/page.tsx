...
export default function TeacherSettingsPage() {
  ...
  const genderLabel =
    teacher?.gender === "M" || teacher?.gender === "Masculin"
      ? "Masculin"
      : teacher?.gender === "F" || teacher?.gender === "Féminin"
        ? "Féminin"
        : teacher?.gender || "—"
    ...
    <div className="space-y-5 lg:space-y-8">
      <div className="pt-2 text-center lg:pt-4">
        <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-indigo-600 text-2xl font-bold text-white lg:h-24 lg:w-24 lg:text-3xl">
          {teacher?.firstName?.charAt(0)?.toUpperCase() || "P"}
        </div>
        <h1 className={cn("text-xl font-bold lg:text-2xl", text)}>{fullName || "Mon profil"}</h1>
        {teacher?.specialty && <p className={cn("mt-1 text-sm lg:text-base", textMuted)}>{teacher.specialty}</p>}
        {teacher && (
        ...
        <>
          <SectionCard icon={User} iconBg="bg-blue-50 dark:bg-blue-500/10" iconColor="text-blue-600" title="Informations officielles">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <InfoField label="Spécialité" value={teacher.specialty || "—"} />
              <InfoField label="Genre" value={genderLabel} />
              <InfoField
                label="Date de naissance"
                value={teacher.birthDate ? new Date(teacher.birthDate).toLocaleDateString("fr-FR") : "—"}
              />
              {teacher.hiredAt && (
                <InfoField
                  label="Date d'embauche"
                  value={new Date(teacher.hiredAt).toLocaleDateString("fr-FR")}
                />
              )}
              {teacher.year && <InfoField label="Année scolaire" value={teacher.year} />}
            </div>
          </SectionCard>

          <SectionCard icon={GraduationCap} iconBg="bg-indigo-50 dark:bg-indigo-500/10" iconColor="text-indigo-600" title="Affectations">
            <div className="mb-3 flex gap-4">
              <div>
                <p className={cn("text-[10px] font-bold uppercase tracking-wider", textMuted)}>Classes</p>
                <p className={cn("text-2xl font-bold", text)}>{teacher.classCount}</p>
              </div>
              <div>
                <p className={cn("text-[10px] font-bold uppercase tracking-wider", textMuted)}>Cours</p>
                <p className={cn("text-2xl font-bold", text)}>{teacher.courseCount}</p>
              </div>
            </div>
            {teacher.assignments.length > 0 ? (
              <div className="space-y-2">
                {teacher.assignments.map((a, i) => (
                  <div key={i} className={cn("rounded-xl border px-3 py-2 text-sm", border, isDark ? "bg-white/5" : "bg-gray-50")}>
                    <span className={cn("font-medium", text)}>{a.subjectName}</span>
                    <span className={textMuted}> · {a.className}</span>
                  </div>
                ))}
              </div>
            ) : (
              <p className={cn("text-sm", textMuted)}>Aucune affectation pour cette année.</p>
            )}
          </SectionCard>
      ...
      <button
        ...
        disabled={loggingOut}
        className="flex w-full items-center justify-center gap-2 rounded-2xl border border-red-200 py-3.5 text-sm font-semibold text-red-600 transition-colors hover:bg-red-50 disabled:opacity-50 dark:border-red-500/30 dark:hover:bg-red-500/10"
      >
