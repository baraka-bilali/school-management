...
export default function TeacherCommuniqueViewPage() {
  ...
  useEffect(() => {
    const fetchCommunique = async () => {
      try {
        const res = await fetch(`/api/teacher/communiques/${params.id}`, { credentials: "include" })
        if (res.ok) {
    ...
    if (params.id) void fetchCommunique()
  }, [params.id])
  ...
  if (notFound || !communique) {
    return (
      <div className="flex min-h-[50vh] flex-col items-center justify-center text-center">
        ...
        <button type="button" onClick={() => router.push("/teacher/messages?tab=communiques")} className="mt-4 text-sm font-semibold text-indigo-600">
  ...
  return (
    <div className="space-y-4">
      <button
        type="button"
        onClick={() => router.push("/teacher/messages?tab=communiques")}
