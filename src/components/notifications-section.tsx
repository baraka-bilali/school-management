...
export default function NotificationsSection({ theme }: NotificationsSectionProps) {
  ...
  const getIconStyle = (type: string) => {
    if (type === "SYSTEM_MESSAGE") return "bg-purple-500/20 text-purple-400"
  ...
  return (
    <div className={`rounded-xl border ${borderColor} overflow-hidden`}>
      ...
      <div className={`px-6 pt-6 pb-4 ${bgCard}`}>
        ...
        {showBroadcastForm && (
          <div className={`mt-4 p-4 rounded-xl border ${theme === "dark" ? "border-purple-700/40 bg-purple-900/10" : "border-purple-200 bg-purple-50"}`}>
            ...
            <textarea
              ...
              maxLength={500}
              rows={3}
              placeholder="Ex : Maintenance programmée ce soir de 22h à 23h…"
            ...
            <div className="flex items-center justify-between mt-2 gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                {broadcastStatus === "error" && (
                  <span className="text-xs text-red-400">Erreur d'envoi</span>
                )}
                <button
                  onClick={sendBroadcast}
                  disabled={broadcastLoading || broadcastMessage.trim().length < 5}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg bg-purple-600 text-white hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  {broadcastLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />}
                  Envoyer
                </button>
              </div>
            </div>
        ...
        <div className="flex items-center justify-between mt-4">
          <label className="flex items-center gap-2.5 cursor-pointer">
            <div className="relative">
              ...
              />
              <div className={`w-10 h-5 rounded-full transition-colors ${showUnreadOnly ? "bg-indigo-600" : theme === "dark" ? "bg-gray-600" : "bg-gray-300"}`} />
              <div className={`absolute left-0.5 top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${showUnreadOnly ? "translate-x-5" : ""}`} />
            </div>
                  ...
                  {items.map((n) => (
                    ...
                    >
                      ...
                      {confirmDeleteId === n.id ? (
                        ...
                        <div className={`flex items-start gap-4 px-5 py-4 ${hoverBg}`}>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-3 mt-1.5">
                              ...
                              {!n.isRead && <span className="text-xs font-medium text-red-400">• Non lu</span>}
                            </div>
                          </div>
                            ...
                            {openMenuId === n.id && (
                              <>
                                ...
                                <div className={`absolute right-0 mt-1 w-48 ${bgCard} rounded-lg shadow-lg border ${borderColor} py-1 z-20`}>
                                  ...
                                  <button
                                    onClick={() => { setConfirmDeleteId(n.id); setOpenMenuId(null) }}
