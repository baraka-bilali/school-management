...
export function SubjectMaximaPanel() {
  ...
  const selectedDegree = useMemo(
    () =>
      degrees.find((d) => buildDegreeKey(d.section, d.level, d.stream) === degreeKey) || null,
    [degrees, degreeKey]
  )
  ...
  return (
    <div className="space-y-5">
      <p className="text-xs text-gray-500 dark:text-gray-400">
        Maxima officiels par matière et degré. En Humanités, les maxima sont définis par
        filière.
      </p>
      ...
      {!cycleForDegree && selectedDegree && (
        <p className="text-sm text-amber-600">
          Aucun cycle ne couvre la section « {selectedDegree.section} ». Vérifiez les cycles
          d&apos;évaluation.
        </p>
      )}
